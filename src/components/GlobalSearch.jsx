...
// ─── Search index — static base from parse-kb.mjs, merged with live operators ───
// so newly-released ops are searchable without a KB re-parse.

const STATIC_INDEX = searchIndexData
...
export default function GlobalSearch() {
  ...
  // ops) so they're searchable without a KB re-parse. Non-blocking.
  useEffect(() => {
    let cancelled = false
    operatorsPromise.then(data => {
      if (cancelled || !data) return
      const have = new Set(STATIC_INDEX.filter(i => i.type === 'operator').map(i => i.key))
      const extra = [...(data.atk || []), ...(data.def || [])]
        .filter(op => !have.has(op.name.toLowerCase().replace(/_/g, ' ')))
        .map(op => ({
          type: 'operator',
          key: op.name.toLowerCase().replace(/_/g, ' '),
          label: op.name.replace(/_/g, ' '),
          sub: `${op.side} · ${op.category}`,
          href: `/operators/${op.name}`,
        }))
      if (extra.length > 0) setIndex([...STATIC_INDEX, ...extra])
    }).catch(() => {})
