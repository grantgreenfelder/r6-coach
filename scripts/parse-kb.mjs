...
function parsePlayer(name, playerIndexText = '') {
  ...
  return {
    ...
    prevSeasonStats: prevSeasonName ? {
      ...
      kda:      prevStatsLookup['kda']            || '—',
      kills:    prevStatsLookup['kills']          || '—',
      deaths:   prevStatsLookup['deaths']         || '—',
      winRate:  prevStatsLookup['win rate']       || '—',
      matches:  prevStatsLookup['matches']        || '—',
      hs:       (prevStatsLookup['hs%'] || prevStatsLookup['hs'] || '—').replace('%', ''),
      esr:      prevStatsLookup['esr']            || '—',
      clutches: prevStatsLookup['clutches won']   || prevStatsLookup['clutches'] || '—',
      clutchWR: (prevStatsLookup['clutch win%']   || prevStatsLookup['clutch wr'] || '—').replace('%', ''),
